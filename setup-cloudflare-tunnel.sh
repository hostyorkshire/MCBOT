...
# Prints the real home directory for USERNAME using getent (works correctly
# for root, whose home is /root — not /home/root).  Falls back to
# /home/USERNAME if getent is unavailable or the user is not found.
# ---------------------------------------------------------------------------
get_user_home() {
    local _user="${1:-root}"
    local _home
...
# ---------------------------------------------------------------------------
# validate_cloudflared_config – pre-flight check before service install
# Usage: validate_cloudflared_config /etc/cloudflared/config.yml
# Returns 0 if all checks pass; exits with error message on any failure.
# Attempts automatic repair when the credentials-file path is wrong but the
# JSON can be found in a well-known candidate location.
# ---------------------------------------------------------------------------
validate_cloudflared_config() {
    local config_path="${1:-/etc/cloudflared/config.yml}"

    echo ""
    info "─── Pre-flight validation of ${config_path} ───"

    # 1. Config file must exist
    if [[ ! -f "${config_path}" ]]; then
        error "Config file not found: ${config_path}"
        error "  → The file must exist before the service can be installed."
        error "  → Re-run this script from Step 5 to regenerate it."
        return 1
    fi
    success "Config file found: ${config_path}"

    # 2. YAML syntax + required-field check via python3 (skip if unavailable)
...
try:
    ...
    print('YAML parse error: ' + str(exc))
    sys.exit(1)
required = ('tunnel', 'credentials-file', 'ingress')
missing  = [f for f in required if f not in cfg]
if missing:
    print('Missing required YAML fields: ' + ', '.join(missing))
    sys.exit(1)
" "${config_path}" 2>&1)" || {
            error "Config YAML validation failed:"
            error "  ${_pyerr}"
            error "  → Fix the error in ${config_path} and re-run the script."
            error "  Common causes: indentation error, missing quotes, wrong field names."
            return 1
        }
        success "Config YAML syntax OK."
    else
        info "python3 not found — skipping YAML syntax check."
    fi

    # 3. Extract and verify the credentials file path
    local creds_path=""
    # Strip leading field name and any surrounding quotes from the value
    creds_path="$(grep -E '^credentials-file:' "${config_path}" \
        | sed 's/^credentials-file:[[:space:]]*//' \
        | tr -d "'\"")" || true

    if [[ -z "${creds_path}" ]]; then
        error "No 'credentials-file:' entry found in ${config_path}."
        error "  → The config must contain:  credentials-file: /etc/cloudflared/<TUNNEL-ID>.json"
        error "  → Re-run from Step 5 to regenerate the config file."
        return 1
    fi

    info "Configured credentials path: ${creds_path}"

    if [[ ! -f "${creds_path}" ]]; then
        info "Credentials file not found at configured path: ${creds_path}"
        info "Attempting automatic repair — searching candidate locations ..."

        # Extract tunnel UUID from config to know which JSON file to look for
        local tunnel_uuid=""
        tunnel_uuid="$(grep -E '^tunnel:' "${config_path}" \
            | sed 's/^tunnel:[[:space:]]*//' \
            | tr -d "'\"")" || true
        ...
        if [[ -n "${tunnel_uuid}" ]]; then
            ...
            local _root_home
            _root_home="$(get_user_home root)"
            local _candidates=(
                "/etc/cloudflared/${tunnel_uuid}.json"
                "${_root_home}/.cloudflared/${tunnel_uuid}.json"
            ...
            for _cand in "${_candidates[@]}"; do
                ...
                if [[ -f "${_cand}" ]]; then
                    found_creds="${_cand}"
                    info "  Found credentials at: ${found_creds}"
                    break
                fi
            done
        else
            info "  Could not parse tunnel UUID from ${config_path} — skipping candidate search."
        fi

        if [[ -n "${found_creds}" ]]; then
            local _dest_creds="/etc/cloudflared/${tunnel_uuid}.json"
            if [[ "${found_creds}" != "${_dest_creds}" ]]; then
                info "Copying credentials to system location: ${_dest_creds}"
                sudo cp "${found_creds}" "${_dest_creds}"
                sudo chown root:root "${_dest_creds}"
                sudo chmod 600 "${_dest_creds}"
                success "Credentials copied to ${_dest_creds}"
            fi
            # Update config.yml to point at the canonical /etc/cloudflared/ path
            sudo sed -i \
                "s|^credentials-file:.*|credentials-file: ${_dest_creds}|" \
                "${config_path}"
            success "Updated credentials-file in ${config_path} → ${_dest_creds}"
            # Re-read the now-corrected path for the final check below
            creds_path="${_dest_creds}"
        else
            error "Credentials file does not exist: ${creds_path}"
            if [[ -n "${tunnel_uuid}" ]]; then
                error "  Searched the following locations and found nothing:"
                local _root_home2
                _root_home2="$(get_user_home root)"
                error "    /etc/cloudflared/${tunnel_uuid}.json"
                error "    ${_root_home2}/.cloudflared/${tunnel_uuid}.json"
                error "    ${HOME}/.cloudflared/${tunnel_uuid}.json"
                error "  Tunnel UUID from config: ${tunnel_uuid}"
            fi
            error "  → Regenerate the credentials file with one of:"
            error "    cloudflared tunnel create <name>"
            error "    cloudflared tunnel login  (then re-create the tunnel)"
            error "  → If the tunnel already exists, run 'cloudflared tunnel list'"
            error "    and verify the UUID matches what is in ${config_path}."
            return 1
        fi
    fi

    if [[ ! -r "${creds_path}" ]]; then
        error "Credentials file is not readable: ${creds_path}"
        error "  → Fix permissions: sudo chmod 600 ${creds_path}"
        return 1
    fi
    success "Credentials file OK: ${creds_path}"

    return 0
}

# ---------------------------------------------------------------------------
# _deploy_system_config – copy config + credentials to /etc/cloudflared/
# and tighten file permissions.  Called from Step 8 before every service
# install or restart so the system files are always up-to-date.
# ---------------------------------------------------------------------------
_deploy_system_config() {
    info "Preparing system cloudflared configuration in /etc/cloudflared/ ..."
    sudo mkdir -p /etc/cloudflared

    # Copy the user-level config to the system location
    sudo cp "${CONFIG_FILE}" /etc/cloudflared/config.yml
    info "Copied config to /etc/cloudflared/config.yml"

    # Copy the credentials JSON next to the config so the service can always
    # find it regardless of which user account starts the service.
    if [[ -f "${CREDS_FILE}" ]]; then
        local _creds_fname
        _creds_fname="$(basename "${CREDS_FILE}")"
        sudo cp "${CREDS_FILE}" "/etc/cloudflared/${_creds_fname}"
        info "Copied credentials to /etc/cloudflared/${_creds_fname}"

        # Rewrite the credentials-file path in the system config so it points
        # to the /etc/cloudflared/ copy rather than the home-directory copy.
        sudo sed -i \
            "s|^credentials-file:.*|credentials-file: /etc/cloudflared/${_creds_fname}|" \
...
and check that the .json file exists for your tunnel."
    ...
    # root so that the tunnel token is not world-readable.
    sudo chmod 600 /etc/cloudflared/config.yml
    if sudo ls /etc/cloudflared/*.json >/dev/null 2>&1; then
        sudo chmod 600 /etc/cloudflared/*.json
    fi
    success "Permissions set to 600 on /etc/cloudflared/ files."
}
...
" 2>/dev/null)" || OLD_TUNNEL_ID=""
            ...
            if [[ -n "${OLD_TUNNEL_ID}" && -f "${_linux_user_home}/.cloudflared/${OLD_TUNNEL_ID}.json" ]]; then
                ...
                rm -f "${_linux_user_home}/.cloudflared/${OLD_TUNNEL_ID}.json"
            fi
            # Also remove from /etc/cloudflared/ if present (idempotent cleanup)
            if [[ -n "${OLD_TUNNEL_ID}" ]]; then
                sudo rm -f "/etc/cloudflared/${OLD_TUNNEL_ID}.json" 2>/dev/null || true
            fi
...
# Determine the real home directory of LINUX_USER so the credentials path is
# correct even when the user is root (whose home is /root, not /home/root).
LINUX_USER_HOME="$(get_user_home "${LINUX_USER}")"
CREDS_FILE="${LINUX_USER_HOME}/.cloudflared/${TUNNEL_ID}.json"
...
# Secure the user-level config and credentials files
...
Options:
  ...
  N  – Skip install, just restart the existing service"
    ...
    if [[ "${REINSTALL_SERVICE}" =~ ^[Yy]$ ]]; then
        ...
        info "Validating configuration before re-installing service..."
        if ! validate_cloudflared_config /etc/cloudflared/config.yml; then
            error "Pre-flight validation failed — service install aborted."
            error "Fix the errors listed above, then re-run this script."
            error "Do NOT attempt 'sudo systemctl start cloudflared' until the"
            error "config and credentials pass validation."
            exit 1
        fi
        ...
        info "Skipping service re-install — refreshing config and restarting..."
        _deploy_system_config

        info "Validating configuration before restarting service..."
        if ! validate_cloudflared_config /etc/cloudflared/config.yml; then
            error "Pre-flight validation failed — service restart aborted."
            error "Fix the errors listed above, then re-run this script."
            exit 1
        fi
    ...
    info "Validating configuration before installing service..."
    if ! validate_cloudflared_config /etc/cloudflared/config.yml; then
        error "Pre-flight validation failed — service install aborted."
        error "Fix the errors listed above, then re-run this script."
        exit 1
