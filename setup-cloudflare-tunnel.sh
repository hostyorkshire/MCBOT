...
" "${config_path}" 2>&1)" || {
    ...
    if [[ ! -f "${creds_path}" ]]; then
        ...
        error "  → This file is generated by: cloudflared tunnel create <name>"
        error "  → If the tunnel was re-created, the old credentials are invalid."
        error "  → Copy the new <TUNNEL-ID>.json to /etc/cloudflared/ and update"
        error "    credentials-file: in ${config_path}, then re-run this script."
        return 1
    fi
