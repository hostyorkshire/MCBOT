...
class Session:
    ...
    Attributes:
        ...
        story_id: Stable UUID4 string that uniquely identifies this story
            session.  Generated on creation so that multiple sessions for the
            same ``user_key`` can be distinguished in the persistent log.
        ...
        awaiting_chapter_choice: ``True`` when the engine is waiting for the
            ...
            boundary.
        awaiting_end_choice: ``True`` when the engine is waiting for the user
            to choose Restart (1), New adventure (2), or Quit (3) after an
            in-story ``[END]`` screen generated by the LLM.
        finished: ``True`` once the story has reached a terminal end and has
            been logged.
        finished_at: Unix timestamp when the story was marked finished, or
    ...
    def __init__(self, user_key: str, user_name: str, max_history: int = 10) -> None:
        ...
        self.awaiting_end_choice: bool = False
...
class StoryEngine:
    ...
    def _session_to_dict(self, session: Session) -> dict:
        ...
        return {
            ...
            "awaiting_end_choice": session.awaiting_end_choice,
    ...
    async def advance_story(self, user_key: str, choice: Any) -> str:
        ...
        *choice* can be ``1``, ``2``, ``3`` (int or str), or free text.
        ...
        2b. If ``awaiting_end_choice`` is set (the LLM produced an in-story
            ``[END]`` screen), interpret the choice as a post-end decision:

           * ``1`` – Restart: log the current session and begin a new story
             with the same user name and genre.
           * ``2`` – New adventure: log the current session and prompt the
             user to send ``start`` to choose a new genre.
           * ``3`` – Quit: mark the story finished, log it, and remove the
             session from memory.
           * Any other input: re-show the end-screen prompt.
        ...
        if session.awaiting_chapter_choice:
            ...
            if digit == "1":
                ...
                if "[END]" in reply:
                    ...
                    log.info(
                        "End screen detected for %s after chapter resume – awaiting end-screen choice",
                        user_key,
                    )
                return reply
        ...
        # ------------------------------------------------------------------
        # End-screen choice handling (after an in-story [END] screen)
        # ------------------------------------------------------------------
        if session.awaiting_end_choice:
            # Normalise to the first character so "1 Restart" etc. also work.
            digit = choice_text[:1]

            if digit == "1":
                # Restart – log the current session and begin a new story
                # with the same user name and genre.
                session.awaiting_end_choice = False
                log.info("Player %s chose to restart (end-screen choice 1)", user_key)
                # start_story will log the existing session and create a fresh one.
                return await self.start_story(user_key, session.user_name, session.genre)

            if digit == "2":
                # New adventure – log the current session and prompt the user
                # to send 'start' to choose a genre.
                session.awaiting_end_choice = False
                _log_story(self._session_to_dict(session))
                log.info("Player %s chose new adventure (end-screen choice 2)", user_key)
                log.debug(
                    "_log_story called (end-screen new-adventure): user_key=%s started_at=%s",
                    user_key,
                    session.started_at,
                )
                self._sessions.pop(user_key, None)
                return "Your tale has been recorded. Send 'start' to begin a new adventure!"

            if digit == "3":
                # Quit – mark finished, log, and remove the session.
                session.awaiting_end_choice = False
                session.finished = True
                session.finished_at = time.time()
                _log_story(self._session_to_dict(session))
                log.info("Story quit by player %s (end-screen choice 3)", user_key)
                log.debug(
                    "_log_story called (end-screen quit): user_key=%s started_at=%s",
                    user_key,
                    session.started_at,
                )
                self._sessions.pop(user_key, None)
                return "Goodbye, adventurer. Your tale has been recorded."

            # Unrecognised input – re-show the end-screen prompt.
            return _END_SCREEN_PROMPT
        ...
        if "[END]" in reply:
            session.awaiting_end_choice = True
            log.info("End screen detected for %s – awaiting end-screen choice", user_key)
        return reply
