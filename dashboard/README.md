...
- The dashboard service loads environment variables (including `GROQ_API_KEY`)
  from the repository `.env` file via `EnvironmentFile=<workdir>/.env`.  Make
  sure your `.env` contains a valid `GROQ_API_KEY` so that the `/chat` endpoint
  can authenticate with Groq.  The `.env` file is created by `setup.sh` and is
